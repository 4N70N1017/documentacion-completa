...
__int64 CDlgCapturarAbono::obtenerPolizaXML(long lPoliza)
{
    ...
    if(sprintf_s(cSql, "SELECT substring(unnest(cast(xpath('/Movimientos/Movimiento/@NoPoliza', xml::xml) as text)::text[]),3,10) FROM ctlcrseguroauto c where c.poliza = %10ld", lPoliza) < 0){;}
