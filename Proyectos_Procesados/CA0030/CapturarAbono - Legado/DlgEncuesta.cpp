...
bool CDlgEncuesta::validarControl(char * cCadena)
{
    ...
    case 0:
        ...
        if (sTexto.IsEmpty())
        {
			if(sprintf_s(cCadena, strnlen(cCadena, 255), "%s", " DEBE CAPTURAR EL NOMBRE DE LA PERSONA NO TITULAR ") < 0){;}
            m_nombre.SetWindowText("");
            valorRegresa = false;
        }
        else if (sTexto.GetLength() <= 2)
        {
			if(sprintf_s(cCadena, strnlen(cCadena, 255), " FAVOR DE INGRESAR AL MENOS TRES CARACTERES AL CAPTURAR EL NOMBRE ") < 0){;}
            m_nombre.SetWindowText("");
            valorRegresa = false;
        }
        else
    ...
    case 1:
        ...
        if (sTexto.IsEmpty())
        {
			if(sprintf_s(cCadena, strnlen(cCadena, 255), "%s", " DEBE CAPTURAR EL APELLIDO PATERNO DE LA PERSONA NO TITULAR ") < 0){;}
    ...
    case 2:
        ...
        if (sTexto.IsEmpty())
        {
			if(sprintf_s(cCadena, strnlen(cCadena, 255), "%s", " DEBE CAPTURAR EL APELLIDO MATERNO DE LA PERSONA NO TITULAR ") < 0){;}
    ...
    case 3:
        ...
        {
			if(sprintf_s(cCadena, strnlen(cCadena, 255), "%s", " CAPTURA INCORRECTA ") < 0){;}
            m_zona.SetWindowText("");
            valorRegresa = false;
        }
        else
        {
            ...
            {
				if(sprintf_s(cCadena, strnlen(cCadena, 255), "%s", " EL NUMERO DE ZONA NO CORRESPONDE A ESTA CIUDAD ") < 0){;}
                m_zona.SetWindowText("");
                valorRegresa = false;
            }
        }
