...
Return
    ...
    UTE_ERROR               - failure
**********************************************/
int CUT_HTTPClient::SetServer(LPSTR name,BOOL bCheckIPAddress){
...
int CUT_HTTPClient::SendPutCommand(LPSTR targetURL, CUT_DataSource &sourceFile, BOOL bPost){
   ...
   if ((error = HTTPConnect()) !=  UTE_SUCCESS)
    ...
    if(m_bUseProxy) {
        // if the url includes http then it is already an absolute URI
        // so lets just format our command string
        if (_strnicmp (targetURL,"http://", strlen("http://"))==0 || _strnicmp (targetURL,"https://", strlen("https://"))==0)
			if (bPost)
...
int CUT_HTTPClient::HTTPConnect()
{
	...
	{
		if((error = Connect( (m_bUseProxy) ? m_nProxyPort : m_nPort, (m_bUseProxy) ? m_szProxyAddr : m_szServerName, m_nConnectTimeout)) != UTE_SUCCESS)
			return OnError(error);
	}
	else
	{
...
/********************************************************
SetHOST()
Especifica el nombre del host apartir del URL
PARAM
	LPSTR url      - URL del servicio
RET
	UTE_SUCCESS             - success
    UTE_PARAMETER_TOO_LONG  - param too long
    UTE_ERROR               - failure
********************************************************/
int CUT_HTTPClient::SetHOST( LPSTR url )
{
	CString sUrl;
	int iIni = 0, iFin = 0;
	char cHost[64] = {0};
	...
	sprintf( cHost, "%s", sUrl.Mid( iIni, iFin - iIni ) );

	return SetServer( cHost, FALSE );
}
